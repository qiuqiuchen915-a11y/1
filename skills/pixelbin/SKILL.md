...
# PixelBin Media Skill

Turn GPT/Codex into a full media pipeline. Generate, transform, store, and deliver images & videos at scale using PixelBin.
...
Read [`INTRO.md`](INTRO.md) before responding. INTRO.md is the user-facing voice of this skill — match its tone and follow its "How GPT/Codex should respond" section.
...
`scripts/seo-content.js` produces `brief.json`. It includes `design_system` (palette / fonts / CSS vars / max-widths) extracted from the brand reference. GPT/Codex then reads the brief and writes `page-spec.json`. `build-page.js` consumes the `design` block in `page-spec.json` and applies it as CSS variables (`--fg`, `--bg`, `--accent`, `--font-body`, `--font-heading`, `--container`).
